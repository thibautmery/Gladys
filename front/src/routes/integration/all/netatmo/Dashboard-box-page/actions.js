...
const createActions = store => {
  const actions = {
    ...
    async updateSetpoint(state,e,props) {
      
      console.log(state)
      console.log(e)
      console.log(this)
      const deviceFeature = this.deviceFeature
      const min = deviceFeature.min
      const max = deviceFeature.max
      console.log(deviceFeature)
      if (e.target.id === "+"){
        deviceFeature.last_value = deviceFeature.last_value + 1
      } else if (e.target.id === "-"){
        deviceFeature.last_value = deviceFeature.last_value - 1
      }
      console.log(deviceFeature.last_value)

      const newState = update(this, {
        deviceFeature: {
          last_value: {
            $set: deviceFeature.last_value
          }
        }
      });
